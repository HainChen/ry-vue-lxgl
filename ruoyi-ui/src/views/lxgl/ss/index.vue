  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="处理状态" prop="handleStatus">
        <el-select v-model="queryParams.handleStatus" placeholder="请选择处理状态" clearable style="width: 150px">
          <el-option
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['lxgl:ss:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="info" icon="el-icon-upload2" size="mini" @click="handleImport"
          v-hasPermi="['lxgl:ss:import']">导入</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          type="warning"
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['lxgl:ss:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="ssList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="家具情况" align="center" prop="furniture" />
      <el-table-column label="赔偿金额" align="center" prop="indemnity" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="家具情况" prop="furniture">
          <el-input v-model="form.furniture" type="textarea" placeholder="请输入内容" />
        </el-form-item>
        <el-form-item label="赔偿金额" prop="indemnity">
          <el-input v-model="form.indemnity" placeholder="请输入赔偿金额" />
        </el-form-item>
        <el-form-item label="处理状态" prop="handleStatus">
...
<script>
import { listSs, getSs, delSs, addSs, updateSs } from "@/api/lxgl/ss";
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/lxgl/ss/importData"
      },
      // 表单参数
      form: {},
      // 表单校验
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSs(this.queryParams).then(response => {
        this.ssList = response.rows;
    ...
    /** 下载模板操作 */
    importTemplate() {
      this.download('/lxgl/ss/importTemplate', {
      }, `furniture_template_${new Date().getTime()}.xlsx`)
    },
    // 文件上传中处理
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateSs(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addSs(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('lxgl/ss/export', {
        ...this.queryParams
      }, `ss_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
