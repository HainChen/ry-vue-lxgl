  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="处理状态" prop="handleStatus">
        <el-select v-model="queryParams.handleStatus" placeholder="请选择处理状态" clearable style="width:130px ;">
          <el-option
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['lxgl:tsg:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="info" icon="el-icon-upload2" size="mini" @click="handleImport"
          v-hasPermi="['lxgl:tsg:import']">导入</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          type="warning"
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['lxgl:tsg:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="tsgList" @selection-change="handleSelectionChange">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="逾期天数" prop="overdueDays">
          <el-input v-model="form.overdueDays" placeholder="请输入逾期天数" />
        </el-form-item>
        <el-form-item label="处理状态" prop="handleStatus">
      ...
      <el-upload ref="upload" :limit="1" accept=".xlsx, .xls" :headers="upload.headers"
        ...
        <div class="el-upload__tip text-center" slot="tip">
          <div class="el-upload__tip" slot="tip">
            <el-checkbox v-model="upload.updateSupport" /> 是否更新已经存在的借阅数据
          </div>
...
<script>
import { listTsg, getTsg, delTsg, addTsg, updateTsg } from "@/api/lxgl/tsg";
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/lxgl/tsg/importData"
      },
      // 表单参数
      form: {},
      // 表单校验
  ...
  methods: {
     /** 导入按钮操作 */
     handleImport() {
      this.upload.title = "借阅信息导入";
      this.upload.open = true;
    },
    /** 下载模板操作 */
    importTemplate() {
      this.download('/lxgl/tsg/importTemplate', {
      }, `borrowed_template_${new Date().getTime()}.xlsx`)
    },
    // 文件上传中处理
    ...
    getList() {
      this.loading = true;
      listTsg(this.queryParams).then(response => {
        this.tsgList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateTsg(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addTsg(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('lxgl/tsg/export', {
        ...this.queryParams
      }, `tsg_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
